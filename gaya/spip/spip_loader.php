 ...
 * 
 * Au choix parmi la liste des branches dÃ©clarÃ©es ('dev', '3.2', '3.1', ...)
 ...
 * (avec lâ€™adresse du zip et la version minimale de PHP)
 * 
 * @param string|null $branche 
 *     Pour retourner lâ€™info dâ€™une branche spÃ©cifique
 * @return array|false 
 *     Descriptif des branches, ou dâ€™une seule branche
 */
 ...
 * - 2.5    : affichage de la version Ã  installer, de la version dÃ©jÃ  installÃ©e (si elle existe),
 * -          compatibilite PHP, loader obsolete
 * - 2.5.10 : on tÃ©lÃ©charge maintenant SPIP 3.2 
 * - 2.5.11 : coquille empechant des mises Ã  jour
 * - 2.6.0  : dÃ©claration simplifiÃ©e des branches / zips + sÃ©lecteur de branche
 * - 3.0.0  : le SPIP loader analyse et dÃ©place les fichiers obsolÃ¨tes dans un rÃ©pertoire 'fichiers_obsoletes_{date}'
 *            pour les rÃ©pertoires appartenant Ã  SPIP (ecrire, prive, plugins-dist, squelettes-dist).
 *            /!\ si vous avez ajoutÃ© des plugins dans plugins-dist, ils seront aussi dÃ©placÃ©s !!
 * - 3.0.1  : le sÃ©lecteur choisit par dÃ©faut la branche actuel du SPIP dÃ©jÃ  installÃ©, sâ€™il la connait.
 * - 3.0.2  : Un rÃ©pertoire obsolÃ¨te nâ€™est pas dÃ©placÃ© sâ€™il contient un fichier '.spip_loader_keep'
 */
...
/** 
 * Notre branche de destination par dÃ©faut 
 * 
 * - ignorÃ© si la constante _CHEMIN_FICHIER_ZIP est forcÃ©e
 * - ignorÃ© si un SPIP est dÃ©jÃ  installÃ© (tentera de rester sur la mÃªme branche par dÃ©faut)
 */
$notre_branche = lister_branches_proposees(_DEFAUT_BRANCHE_MAJ);
if (!$notre_branche) {
	die("Mauvaise dÃ©finition de la constante _DEFAUT_BRANCHE_MAJ. <code>Branche " . _DEFAUT_BRANCHE_MAJ . " inconnue</code>");
}


if (!defined('_CHEMIN_FICHIER_ZIP')) {
	/**
	 * Chemin du zip installÃ© par dÃ©faut
	 * 
	 * Si la constante _CHEMIN_FICHIER_ZIP est dÃ©jÃ  dÃ©finie, 
	 * alors le zip dÃ©fini sera utilisÃ©.
	 * 
	 * Sinon, on prend par dÃ©faut le zip de la branche installÃ©e par dÃ©faut.
	 */
	define('_CHEMIN_FICHIER_ZIP', $notre_branche['zip']);
} else {
	// Ã©viter dâ€™afficher le sÃ©lecteur de branche dans ces cas lÃ .
...
/** 
 * Le SPIP Loader ne place pas dans le rÃ©pertoire obsolÃ¨te
 * un rÃ©pertoire qui contiendrait un fichier avec ce nom.
 */
...
/**
 * Affiche un sÃ©lecteur de menu pour choisir le zip (la branche) Ã  utiliser.
 * 
 * @param array $active Chemin du paquet Ã  tÃ©lÃ©charger actuellement sÃ©lectionnÃ©
 * @param string $version_installee Version de SPIP actuellement installÃ©e
 * @return string
 */
...
/**
 * DÃ©place les fichiers qui sont en trop entre le contenu du zip et le rÃ©pertoire destination.
 * 
 * @param array $content Liste des fichiers issus de pclZip
 * @param string $dir RÃ©pertoire oÃ¹ ils ont Ã©tÃ© copiÃ©s.
 */
...
/**
 * Retourne la liste des fichiers/rÃ©pertoires en trop entre le zip et la destination,
 * pour certains rÃ©pertoires seulement.
 */
function comparer_contenus($content, $dir) {
	...
	// On se considÃ¨re dans SPIP et on vÃ©rifie seulement ces rÃ©pertoires
...
/**
 * Liste les contenus en trop dans certains rÃ©pertoires, en fonction dâ€™une liste de fichiers 
 * 
 * Un rÃ©pertoire superflu, mais contenant un fichier .spip_loader_keep est conservÃ©,
 * c'est Ã  dire quâ€™il ne sera pas retournÃ© dans cette liste de fichiers/rÃ©pertoire obsolÃ¨tes.
 * 
 * @param array $contenus_source liste(chemin => isDir?)
 * @param string $dir Chemin du rÃ©pertoire Ã  tester
 * @param string $base RÃ©pertoire en cours dâ€™analyse (pour rÃ©cursivitÃ©)
 * @param array|null $repertoires_testes Liste de rÃ©pertoires Ã  uniquement parcourrir si dÃ©fini.
 * @return array liste(chemin => isDir?) des fichiers/rÃ©pertoire en trop.
 */
function lister_contenus_superflus($contenus_source, $dir, $base, $repertoires_testes = null) {
	...
	if ($dh = opendir($dir)) {
		while (($file = readdir($dh)) !== false) {
			...
			$_base = $base . $file;
			if (is_dir($dir . '/' . $file)) {
				// rÃ©pertoire prÃ©sent ou en trop ?
				if (!isset($contenus_source[$_base . '/'])) {
					// ne pas rendre obsolÃ¨te si un fichier de conservation est prÃ©sent.
...
function spip_presente_deballe($fichier, $paquet, $dest, $range) {
	...
	if ($php_incompatible) {
		...
		// Ã‰pargnons un downgrade aux personnes Ã©tourdies
		$bouton =
