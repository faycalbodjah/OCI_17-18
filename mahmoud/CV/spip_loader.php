 ...
 * - 2.5    : affichage de la version �  installer, de la version déj�  installée (si elle existe),
 * -          compatibilite PHP, loader obsolete
 * - 2.5.10 : on télécharge maintenant SPIP 3.2 
 * - 2.5.11 : coquille empechant des mises �  jour
 ...
 *            pour les répertoires appartenant �  SPIP (ecrire, prive, plugins-dist, squelettes-dist).
 *            /!\ si vous avez ajouté des plugins dans plugins-dist, ils seront aussi déplacés !!
 * - 3.0.1  : le sélecteur choisit par défaut la branche actuel du SPIP déj�  installé, s’il la connait.
 ...
 * - ignoré si un SPIP est déj�  installé (tentera de rester sur la même branche par défaut)
 */
...
if (!defined('_CHEMIN_FICHIER_ZIP')) {
	 ...
	 * 
	 * Si la constante _CHEMIN_FICHIER_ZIP est déj�  définie, 
	...
	// éviter d’afficher le sélecteur de branche dans ces cas l� .
...
//
// Renvoie un tableau des versions SPIP dont l'index correspond �  au chemin du fichier zip tel
...
/**
 * Affiche un sélecteur de menu pour choisir le zip (la branche) �  utiliser.
 * 
 * @param array $active Chemin du paquet �  télécharger actuellement sélectionné
 ...
 * c'est �  dire qu’il ne sera pas retourné dans cette liste de fichiers/répertoire obsolètes.
 * 
 * @param array $contenus_source liste(chemin => isDir?)
 * @param string $dir Chemin du répertoire �  tester
 * @param string $base Répertoire en cours d’analyse (pour récursivité)
 * @param array|null $repertoires_testes Liste de répertoires �  uniquement parcourrir si défini.
...
function spip_presente_deballe($fichier, $paquet, $dest, $range) {
	...
	// Version proposée �  l'installation par défaut
