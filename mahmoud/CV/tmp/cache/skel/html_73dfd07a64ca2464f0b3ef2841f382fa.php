 ...
 * Compile :   Wed, 07 Mar 2018 11:49:23 GMT
...
(($t1 = strval(interdire_scripts(((($Pile[$SP-1]['mode'] == 'document')) ?' ' :''))))!=='' ?
		($t1 . (	'<div class="type">
			' .
	interdire_scripts(((($a = supprimer_numero(typo($Pile[$SP]['titre']), "TYPO", $connect, $Pile[0])) OR (is_string($a) AND strlen($a))) ? $a : concat($l1,' ',interdire_scripts(majuscules($Pile[$SP]['extension']))))) .
	'
				</div>
			')) :
		'') .
...
'
			...
			' .
(($t1 = strval(interdire_scripts(((($a = supprimer_numero(typo($Pile[$SP]['titre']), "TYPO", $connect, $Pile[0])) OR (is_string($a) AND strlen($a))) ? $a : (	'<i class="sanstitre">' .
	$l1 .
	'</i>')))))!=='' ?
